...
def get_logger(
    ...
    if base_path:
        ...
        file_handler = logging.FileHandler(logfile_folder / "sphinx_docs.log", mode="w", encoding="utf8")
...
def run_publish_on_github(built_docs_dir: Path, remote: str = "origin", dryrun: bool = False) -> int:
    ...
    if not built_docs_dir.is_dir():
        LOGGER.error("Document source does not exist. Build the html document before you continue.")
        return 2
    ...
    LOGGER.info("Add .nojekyll to root folder. This is required for css rendering on pages.")
    (tmp_dir / ".nojekyll").touch()
    try:
        LOGGER.info("Commit all the changes.")
        git_repo.add(".")
        git_repo.commit("--no-verify", "-m", f"new pages version {datetime.datetime.now()}")
        if not dryrun:
